...
export default function SendFileSMS() {
  ...
  const { sendFileSMS, getTemplates, getSenderIds } = useClientHook();
  ...
  const handleSendSMS = async () => {
    console.log("called");
    setProcessing(true);
    if (!uploadedFile || !message || !selectedSenderId) {
      toast({
        title: "Error",
    ...
    try {
      ...
      let res = await sendFileSMS(formData);
      if(res.data.message !== "success")  throw new error(res.data.message);
      toast({
        title: "Send Bulk SMS",
        description: "Sent SMS successfully!",
      });
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
                ...
                {senderIds
                  ...
                  .map((sender) => (
                    <div
                      ...
                      onClick={() =>
                        sender.status === "approved" &&
                        setSelectedSenderId(sender.name)
                      }
                    >
                      <div className="flex items-center justify-between">
            ...
            <Card className="border-0 shadow-2xl bg-gradient-to-br from-white to-green-50 rounded-3xl overflow-hidden">
              ...
              <CardContent className="p-6 space-y-4">
                {templates
                  .filter((tem) => tem.status == "approved")
                  .map((template) => (
                    <div
                      key={template.id}
                      className={`p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 ${
