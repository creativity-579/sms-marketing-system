...
export default function GroupManagement() {
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      if(res.data.message !== "success") throw new error(res.data.error);
      toast({
  ...
  const handleCSVGroupUpload = async () => {
    ...
    try {
      let res = await createGroup(formData, { isCSV: true });
      if(res.data.message !== "success") throw new error(res.data.message);
      toast({
        title:"Create New Group",
        description: "Created New Group From Upload successfully!"
      })

      setCsvFile(null);
      setNewGroup({ name: "", description: "", numbers: "", file: "" });
      getAllGroup();
    } catch (error) {
