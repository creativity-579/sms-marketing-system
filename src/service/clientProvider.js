...
const clientService = {
  ...
  createGroup: async (newGroup, options) => {
    try {
      ...
      if (options.isCSV == true) {
        const res = await axios.post(authService.url + "/groups/file", newGroup);
        if (res.data.message !== "success") throw new error(res.data.message);
        return res;
      }
  ...
  retryMessage: async (id) => {
    try {
      await authService.setToken();
      const res = await axios.put(authService.url + `/queue/${id}`, { action: "retry" });
      if (res.data.message !== "success") throw new error(res.data.message);
      return res;
    } catch (error) {
      console.log("error", error.message);
  ...
  updateProfile: async (updateData) => {
    try {
      await authService.setToken();
      const res = await axios.put(
        authService.url + "/account/profile",
        updateData
      );
      if (res.data.message !== "success") throw new error(res.data.message);
      return res;
  ...
  updatePassword: async (updateData) => {
    try {
      await authService.setToken();
      const res = await axios.put(
        authService.url + "/account/password",
        updateData
      );
      if (res.data.message !== "success") throw new error(res.data.message);
